...
//    boolean isGrid = true;
    ...
    public AdmobNativeAds(Activity ctx){
        this.ctx = ctx;
        this.act = ctx;
        ...
        //ADMOB_AD_UNIT_ID = ctx.getString(R.string.admob_native_xml_id);
        ADMOB_AD_UNIT_ID = ctx.getString(R.string.admob_express_small_id);
    }
    ...
    public void refreshAd(final FrameLayout frameLayout, boolean requestAppInstallAds, boolean requestContentAds, final int layoutType) {
        ...
        AdLoader adLoader = builder.withAdListener(new AdListener() {
            ...
            public void onAdFailedToLoad(int errorCode) {
                loadError = true;
                if (log>1) Toast.makeText(((ActivityTemplate)act), "AM Failed to load native ad: " + errorCode, Toast.LENGTH_SHORT).show();
                if (nativeAd!=null)
            ...
            public void onAdLoaded() {
                /*
                if (log>3) Toast.makeText(((ActivityTemplate)act), "Ad mob loaded", Toast.LENGTH_SHORT).show();
                if (nativeAd!=null)
                ...
                */
                loadError = false;
                if (log>1) Toast.makeText(ctx, "Admob native loaded!", Toast.LENGTH_SHORT).show();
